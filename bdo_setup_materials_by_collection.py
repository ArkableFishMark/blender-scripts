...
def clean_asset_name(s):
    ...
    while True:
        ...
        if not ext: break
        if re.match(r'^\.\d+$', ext): s = base; continue
        if ext in {'.png', '.jpg', '.jpeg', '.dds', '.tga', '.tif', '.tiff', '.bmp'}: s = base; continue
        break
    return s

def is_target_suffix(node, suffix):
    suffix = suffix.lower()
    if clean_asset_name(node.name).endswith(suffix): return True
    if getattr(node, 'image', None):
        if clean_asset_name(node.image.name).endswith(suffix): return True
        if clean_asset_name(node.image.filepath).endswith(suffix): return True
    return False

def is_target_contains(node, substring):
    substring = substring.lower()
    if substring in clean_asset_name(node.name): return True
    if getattr(node, 'image', None):
        if substring in clean_asset_name(node.image.name): return True
        if substring in clean_asset_name(node.image.filepath): return True
    return False

def has_separate_color_setup(node):
    if 'Color' in node.outputs:
        for link in node.outputs['Color'].links:
            if link.to_node.type in {'SEPARATE_COLOR', 'SEPRGB'}: return True
    return False

def has_bump_setup(node):
    if 'Alpha' in node.outputs:
        for link in node.outputs['Alpha'].links:
            if link.to_node.type == 'BUMP': return True
    return False

def has_normal_map_setup(node):
    if 'Color' in node.outputs:
        for link in node.outputs['Color'].links:
            if link.to_node.type == 'NORMAL_MAP': return True
    return False
...
def auto_load_textures_from_dir(mat, base_node):
    if not base_node.image or not base_node.image.filepath: return
    ...
    for filename in os.listdir(dir_path):
        ...
        if not any(ext in filename.lower() for ext in valid_extensions): continue
        ...
        if not already_loaded:
            try:
                ...
                if target_suffix == '_n': new_node.location = (bx, by - 320)
                elif target_suffix == '_sp': new_node.location = (bx, by - 640)
                elif target_suffix == 'hairspecshift': new_node.location = (bx - 350, by - 640)
...
def process_single_material(mat):
    ...
    if base_color_input and base_color_input.is_linked:
        from_node = base_color_input.links[0].from_node
        if from_node.type == 'TEX_IMAGE': auto_load_textures_from_dir(mat, from_node)
        
    for node in list(nodes): 
        if node.type == 'TEX_IMAGE':
            if getattr(node, 'interpolation', None) == 'Linear': node.interpolation = 'Cubic'
            ...
            if is_n or is_sp or is_hair_spec:
                ...
                if node.image: node.image.colorspace_settings.name = 'Non-Color'
            ...
            if is_n:
                if not has_normal_map_setup(node):
                    ...
                    if bpy.app.version >= (5, 0, 0) and hasattr(nm_node, 'convention'): nm_node.convention = 'DIRECTX'
            ...
            if is_sp or is_hair_spec:
                if 'Specular Tint' in bsdf.inputs: bsdf.inputs['Specular Tint'].default_value = (0.33, 0.33, 0.33, 1.0)
                if has_separate_color_setup(node): continue 
                x, y = node.location
                ...
                if is_sp:
                    ...
                    if 'Roughness' in bsdf.inputs: links.new(bc_node.outputs['Color'], bsdf.inputs['Roughness'])
                elif is_hair_spec:
                    if 'Roughness' in bsdf.inputs: links.new(sep_color.outputs['Red'], bsdf.inputs['Roughness'])
                        
                spec_input = bsdf.inputs.get('Specular IOR Level') or bsdf.inputs.get('Specular')
                if spec_input: links.new(sep_color.outputs['Green'], spec_input)
                if 'Metallic' in bsdf.inputs: links.new(sep_color.outputs['Blue'], bsdf.inputs['Metallic'])

            if is_hair_base:
                if has_bump_setup(node): continue 
                x, y = node.location
                bump_node = nodes.new(type="ShaderNodeBump")
                bump_node.location = (x + 300, y - 250)
                bump_node.inputs['Distance'].default_value = 0.0075
                if 'Alpha' in node.outputs: links.new(node.outputs['Alpha'], bump_node.inputs['Height'])
                if 'Normal' in bsdf.inputs:
                    normal_input = bsdf.inputs['Normal']
                    if normal_input.is_linked:
                        target_node = normal_input.links[0].from_node
        ...
        if node.type == 'NORMAL_MAP' and bpy.app.version >= (5, 0, 0):
            if hasattr(node, 'convention') and node.convention != 'DIRECTX': node.convention = 'DIRECTX'

def get_all_objects_in_collection(collection, objects_set):
    for obj in collection.objects: objects_set.add(obj)
    for sub_col in collection.children: get_all_objects_in_collection(sub_col, objects_set)

def setup_materials_in_collection(target_collection):
    objects = set()
    get_all_objects_in_collection(target_collection, objects)
    ...
    for obj in objects:
        if obj.type in {'MESH', 'CURVE', 'SURFACE', 'META', 'FONT'}:
            for slot in obj.material_slots:
                if slot.material:
                    if slot.material in processed_materials: continue
                    process_single_material(slot.material)
                    processed_materials.add(slot.material)

active_collection = bpy.context.view_layer.active_layer_collection.collection
if active_collection:
    setup_materials_in_collection(active_collection)
    print(f"Collection folder framework processed: '{active_collection.name}'")
else:
    print("Warning: Active Outliner collection context not resolved.")
